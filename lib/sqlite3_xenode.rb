...
class SQLite3Xenode
  ...
  def startup()
    ...
    begin
      ...
      # default to disk_dir path
      # @rel_path = @config[:rel_path]
      # @rel_path ||= true
      
      # default db_path
      @sql_lite_db_path = @config[:db_path]
      @sql_lite_db_path ||= "slqte3.db"
    
      # convenience 
      do_debug("#{mctx} - config: #{@config.inspect}", true)
      
      # Open the local database
      fp = File.expand_path(File.join(@disk_dir, @sql_lite_db_path))
      do_debug("#{mctx} - db path: #{fp.inspect}", true)
      @db = SQLite3::Database.new fp
      
      # if @rel_path
      #   fp = File.expand_path(File.join(@disk_dir, @sql_lite_db_path))
      #   do_debug("#{mctx} - db path: #{fp.inspect}")
      #   @db = SQLite3::Database.new fp
      # else
      #   @db = SQLite3::Database.new @sql_lite_db_path.to_s
      #   do_debug("#{mctx} - db path: #{@sql_lite_db_path.inspect}")
      # end
      
      if @db
        
        begin
          sql = "CREATE TABLE Shipping(purchaseordernumber INT,orderdate VARCHAR(20),partnumber VARCHAR(50),productname VARCHAR(100),quantity INT,usprice FLOAT,shipdate VARCHAR(20),shipmethod VARCHAR(100),comment VARCHAR(100),arrivaldate VARCHAR(20))"
          ret = @db.execute(sql)
          do_debug("#{mctx} - table created.", true)
        rescue
          do_debug("#{mctx} - table exists.", true)
        end
  ...
  def process_message(msg)
   mctx = "#{self.class}.#{__method__} [#{@xenode_id}]"
    ...
    do_debug("#{mctx} - Message from parent node: #{msg}")
    do_debug("#{mctx} - Context from parent node: #{msg.context}")
    do_debug("#{mctx} - Data from parent node: #{msg.data}")
    ...
    sql_cmd = ""
    fp = File.join(File.dirname(__FILE__), '..', @config[:sql_template])
    do_debug("#{mctx} - templates path: #{fp.inspect}", true)
    File.open(fp, "r").each_line do |line|
      sql_cmd << line.gsub(/\n/, ' ')
    end
